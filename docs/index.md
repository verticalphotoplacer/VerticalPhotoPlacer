...
## Quick Start
The default VPP user interface requires only one input to work, which is fullpath of the photos folder (See figure 1 below). User could either drag and drop/or browse to the input folder. In this mode, VPP performs Quick view which uses default parameters as available in each photo's metadata. This is done automatically. No other action is needed.  
